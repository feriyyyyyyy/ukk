...
<body>
    ...
    <nav>
        ...
        <div class="menu">
            <div class="menu-item"><button onclick="window.location='{{ route('alumni.dashboard') }}';">Home</button></div>
            <div class="menu-item"><button onclick="window.location='{{ route('alumni.Dataalumni.index') }}';">Data Alumni</button></div>
            <div class="menu-item"><button onclick="window.location='{{ route('tracerstudy.create') }}';">Tracer Study</button></div>
            <div class="menu-item"><button onclick="window.location='{{ route('alumni.tracerkuliah.create') }}';">Tracer Kuliah</button></div>
            <div class="menu-item"><button onclick="window.location='{{ route('alumni.tracerkerja.create') }}';">Tracer Kerja</button></div>
            <div class="menu-item"><button onclick="window.location='{{ route('testimoni.create') }}';">Testimoni</button></div>
        </div>
        <div class="menu_dropdown">
            <button class="burger-icon" id="burgerMenu"><img src="{{ asset('icons/dropdown.png') }}" alt="Dropdown"></button>
            <ul class="dropdown" id="dropdownMenu">
                <form id="logout-form" action="{{ route('alumni.profile.index') }}" method="GET" style="display: inline;">
                    @csrf
                    <button type="submit" class="dropdown-icon"><img src="{{ asset('icons/profile.png') }}" alt="Profile"></button>
                </form>
                <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: inline;">
                    @csrf
                    <button type="submit" class="dropdown-icon"><img src="{{ asset('icons/logout.png') }}" alt="Logout"></button>
                </form>
            </ul>
        </div>
    </nav>
                ...
                <tbody>
                    @if ($alumni)
                        <tr>
                            ...
                            <td>{{ $alumni->no_hp }}</td>
                            <td>{{ $alumni->email }}</td>
                            <td><a href="{{ route('profile.alumni', $alumni->id_alumni) }}" class="btn btn-primary">Edit</a></td>
                        </tr>
                    @else
                        <tr>
                ...
                <tbody>
                    @if ($tracerKuliah)
                        <tr>
                            ...
                            <td><a href="{{ route('profile.kuliah', $tracerKuliah->id_tracer_kuliah) }}" class="btn btn-primary">Edit</a></td>
                        </tr>
                    @else
                        <tr>
                ...
                <tbody>
                    @if ($tracerKerja)
                        <tr>
                            ...
                            <td><a href="{{ route('profile.kerja', $tracerKerja->id_tracer_kerja) }}" class="btn btn-primary">Edit</a></td>
                        </tr>
                    @else
                        <tr>
